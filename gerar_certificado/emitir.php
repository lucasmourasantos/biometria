...
function validacao($cpf, $nome, $curso, $codigo, $conn){
...
//    }
    ...
    function viewTable($conn) {
        ...
        $stmt1 = $conn->query("SELECT COUNT(hora) as result FROM ponto WHERE curso_id = '" . $id_curso . "' and participante_id = '" . $id_participante . "'");
        $result = null;
        ...
        if ($result >= 1) {
            ...
            $stmt = $conn->query("select p.cpf, p.nome, c.nome as curso, c.ch, c.inicio, c.fim, e.nome as evento, 
...
//        $curso = "Workshop Segurança da Informação";
            ...
            validacao($cpf, $nome, $curso, $codigo, $conn);

            $this->Image("layout_ass/" . $layout, 0, 0, 299);
            $image1 = "layout_ass/" . $ass;
            ...
            echo '<script language="javascript">';
            echo 'location.href="../gerar_certificado.php";';
            echo '</script>';
        }
    }
